...
The installed product has five runtime components: the trusted native viewer EXE, a zero-capability AppContainer import-worker EXE, a lazily started AppContainer OpenUSD compatibility-host EXE, a per-generation AppContainer OCCT STEP host EXE, and the Explorer thumbnail COM DLL. The viewer links none of the third-party format-parsing or decoding libraries. Every parser and decoder — fastgltf, the STL/PLY product parsers, ufbx, lib3mf, TinyUSDZ, the pinned Draco decoder, the KTX/Basis transcoder, libwebp, and DirectXTex/WIC — runs only inside `Preview3DImportWorker.exe`. `Preview3DImportHost.exe` additionally links OpenUSD and its pinned app-local support payload for USD composition outside the TinyUSDZ subset, and `Preview3DStepHost.exe` links the constrained OCCT closure for the bounded static STEP/STP subset. The thumbnail provider links its own copy of the bounded fast-path parsers for in-process CPU sampling under Shell's separate process isolation; it shares no process, memory, or cache with the viewer or any import process.
